...
songs.forEach((s, i) => {
  ...
  el.innerHTML = `
    ...
    <b>${s[0]}</b>
    <span class="hide-sm">AI Sound Studio</span>
    <span class="hide-sm">${s[1]}</span>
    <span>${s[2]}</span>
    <span class="play-link" data-index="${i}">Phát</span>
  `;
...
async function toggleAudio() {
  try {
    ...
    toast('Không thể phát bài Ánh sáng và bóng tối. Hãy kiểm tra file MP3.');
  }
}
...
document.querySelectorAll('.play-link').forEach((x) => {
  x.onclick = () => {
    if (x.dataset.index === '0') {
      toggleAudio();
    } else {
      toast('Bài này chưa có file âm thanh.');
    }
  };
});
